...
detected = {(key, int(value)) for (key, value) in map(lambda line: re.match(r'(\w+): (\d+)', line).groups(), MACHINE.splitlines())}
sues = [{(key, int(value)) for (key, value) in re.findall(r'(\w+): (\d+)', line)} for line in INPUT.splitlines()]
detected_dict = dict(list(detected))

for i, sue in enumerate(sues):
    if len(sue - detected) == 0:
        print "Part One:", i + 1

def property_matches((key, value)):
    if key in ['cats', 'trees']:
        return value > detected_dict[key]
...
for i, sue in enumerate(sues):
    if reduce(lambda a, b: a and b, map(property_matches, sue)):
        break
