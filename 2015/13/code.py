...
for line in INPUT.splitlines():
    first_person, action, amount, second_person = re.match(r'(\w+) would (gain|lose) (\d+) happiness units by sitting next to (\w+)\.', line).group(1, 2, 3, 4)
    PEOPLE.add(first_person)
    amount = int(amount)
    if action == "lose":
        amount *= -1
    WEIGHTINGS[(first_person,second_person)] = amount

def determine_happiness(people):
    return sum(WEIGHTINGS[(people[i], people[i+1])] + WEIGHTINGS[(people[i+1], people[i])] for i in range(-1, len(people) - 1))

print "Part One:", max(map(determine_happiness, permutations(PEOPLE)))

for person in PEOPLE:
    WEIGHTINGS[("Me", person)] = 0
    WEIGHTINGS[(person, "Me")] = 0
PEOPLE.add("Me")

print "Part Two:", max(map(determine_happiness, permutations(PEOPLE)))
