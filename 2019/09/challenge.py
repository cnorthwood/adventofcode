...
class IntcodeVM:
    ...
    def _op_input(self, modes):
        self._mem_write(self._memory[self._pc + 1], modes[0], self.inputs.popleft())
        self._pc += 2

    def _op_output(self, modes):
        self.outputs.append(self._mem_read(self._pc + 1, modes[0]))
        self._pc += 2
...
p1_outputs = deque()
IntcodeVM(INPUT, deque([1]), p1_outputs).run()
print(f"Part One: {p1_outputs.popleft()}")
p2_outputs = deque()
IntcodeVM(INPUT, deque([2]), p2_outputs).run()
print(f"Part Two: {p2_outputs.popleft()}")